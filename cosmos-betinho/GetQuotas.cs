...
namespace cosmos_betinho
{
    ...
    {
        [FunctionName("GetQuotas")]
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "get/quotas/{offset:int}/{limit:int}")] HttpRequest req,
            int offset, int limit, ILogger log)
        {
            var email = req.Query["email"];

            QueryDefinition query = new QueryDefinition($"SELECT * FROM c WHERE c.email = '{email}' OFFSET {offset} LIMIT {limit}");
